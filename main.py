...
import math
# from sys import argv as parampampam
import json_writer

from multiprocessing import Process
import time

mrJason1 = 'questions1.csv'
mrJason2 = 'questions2.csv'
mrJason3 = 'questions3.csv'
mrJason4 = 'questions4.csv'
        ...
        while (success == False):
            try:
                ...
                if (parsedQuestion != None):
                    csv_writer.writeToFile(mrJason1, parsedQuestion)
                success = True
            except:
    ...
    for questionId in range(startId, endId):
        ...
        while (success == False):
            try:
                parsedQuestion = parser.getParsedQuestion(questionId)
                if (parsedQuestion != None):
                    csv_writer.writeToFile(mrJason2, parsedQuestion)
                success = True
            except:
    ...
    for questionId in range(startId, endId):
        ...
        while (success == False):
            try:
                parsedQuestion = parser.getParsedQuestion(questionId)
                if (parsedQuestion != None):
                    csv_writer.writeToFile(mrJason3, parsedQuestion)
                success = True
            except:
    ...
    for questionId in range(startId, endId):
        ...
        while (success == False):
            try:
                parsedQuestion = parser.getParsedQuestion(questionId)
                if (parsedQuestion != None):
                    csv_writer.writeToFile(mrJason4, parsedQuestion)
                success = True
            except:
...
if __name__ == "__main__":
    csv_writer.createHeader(mrJason1)
    csv_writer.createHeader(mrJason2)
    csv_writer.createHeader(mrJason3)
    csv_writer.createHeader(mrJason4)
    ...
    countGoes = 47
    countLess = endId - startId + 1
    countLoop = math.ceil((countLess / 4) / countGoes)
    actualLength = countLoop * countGoes
    endId = startId + actualLength - 1
    print("фактический интервал: с {0:d} по {1:d}".format(startId, endId + actualLength * 3))


    # questionQnaParser1(startId + j * countGoes, firstPage + j * countGoes + countGoes)
    # questionQnaParser2(startId + countGoes * countLoop, startId+countLoop + countGoes * countLoop)
    # questionQnaParser3(startId + countGoes * countLoop * 2, startId+countLoop + countGoes * countLoop * 2)
    # questionQnaParser4(startId + countGoes * countLoop * 3, startId+countLoop + countGoes * countLoop * 3)

    # Первый этап
    # ПАРСИНГ ВОПРОСОВ
    clc = time.time()
    for j in range(countLoop):
        t1 = Process(target=questionQnaParser1, args=(startId + j * countGoes, startId + j * countGoes + countGoes,))
        ...
        t3.close();
        t4.close()
        print("загружено вопросов: {0:.5f}%".format(100*(j + 1) / countLoop))
    ...
    print("вопросы отпарсились за {0:.4f} секунд".format(time.time() - clc))
    #
    ...
    # for j in range(countLoop):
    #     t5 = Process(target=usersQnaParserPepare1,
    #                  args=(firstPage + j * countGoes, firstPage + j * countGoes + countGoes,))
    #     t6 = Process(target=usersQnaParserPepare2, args=(firstPage + j * countGoes + countGoes * countLoop,
    #                                                      firstPage + j * countGoes + countGoes + countGoes * countLoop,))
    #     t7 = Process(target=usersQnaParserPepare3, args=(firstPage + j * countGoes + countGoes * countLoop * 2,
    #                                                      firstPage + j * countGoes + countGoes + countGoes * countLoop * 2,))
    #     t8 = Process(target=usersQnaParserPepare4, args=(firstPage + j * countGoes + countGoes * countLoop * 3,
    #                                                      firstPage + j * countGoes + countGoes + countGoes * countLoop * 3,))
    #     t5.start();
    #     t6.start();
    #     t7.start();
    #     t8.start()
